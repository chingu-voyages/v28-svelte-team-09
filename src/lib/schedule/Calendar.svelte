<script>
  ...
  $: lastDayOfTheMonthWeekday = dayjs(currentMonthDays[currentMonthDays.length-1].date).day();
  ...
  $: prevMonthDays = new Array(firstDayOfTheMonthWeekday).fill(dayjs(month.subtract('1', 'month')).endOf("month"))
    .map((day, i) => {
      return {
      dayOfMonth: day.add(i+1-firstDayOfTheMonthWeekday, "day").format('DD')
      };
    });

  $: nextMonthDays = new Array(6 - lastDayOfTheMonthWeekday).fill(dayjs(month.add('1', 'month')).startOf("month"))
    .map((_, i) => {
      return {
      dayOfMonth: i + 1
      };
...
{#if open}
  <div class="z-10 px-2 py-2 w-auto text-base cursor-default"
  ...
  >
    ...
    <div class="grid grid-cols-7">
      ...
      {#each currentMonthDays as day}
        <p on:click={() => firstDayOfWeek = day.date} class="cursor-pointer">{day.dayOfMonth}</p>
      {/each}
      {#each nextMonthDays as day}
        <p class="text-gray-400">{day.dayOfMonth}</p>
      {/each}
    </div>
    <p class="mt-2 cursor-pointer" on:click={() => firstDayOfWeek = dayjs()}>Return to current date</p>
  </div>
{/if}